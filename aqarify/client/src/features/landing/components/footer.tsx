...
export function Footer() {
  ...
  return (
    <footer className="bg-[#0a0a0a] text-white/40 border-t border-white/5">
      <div className="max-w-screen-xl mx-auto px-6 py-10">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          ...
          <div>
            ...
            <p className="text-[9px] font-medium tracking-[0.2em] text-white/30 uppercase mt-1">
          ...
          <nav className="flex flex-wrap gap-x-8 gap-y-2">
            {[
              ...
              <Link key={to + label} to={withTenant(to)}
                className="text-[10px] font-medium tracking-widest uppercase text-white/40 hover:text-white/70 transition-colors">
                {label}
              </Link>
            ))}
          </nav>
